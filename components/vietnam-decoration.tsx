...
export default function VietnamFlagDecoration({
  variant = "light",
  density = "medium", // Changed default to medium
  className = "",
  ...
  useEffect(() => {
    ...
    const opacity = variant === "light" ? 0.15 : 0.2 // Increased opacity

    // Define number of elements based on density
    let flagCount = 8 // Increased base count
    if (density === "medium") flagCount = 12
    if (density === "high") flagCount = 18
    ...
    class Flag {
      ...
      constructor() {
        ...
        this.size = 40 + Math.random() * 60 // Increased size
